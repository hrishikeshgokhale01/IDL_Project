...
from dataloader_with_noise import LibriSpeechDatasetWithNoise, custom_collate_fn
...
dataset = LibriSpeechDatasetWithNoise(root_dirs=root_dirs, ratio = 0.2)
...
with open(os.path.join(model_save_path, "model_local_2_summary.txt"), "w") as f:
    f.write(str(model))
...
for epoch in range(num_epochs):
    ...
    model_file = os.path.join(model_save_path, f"conformer_local_2_epoch_{epoch + 1}.pth")
...
plt.legend()
plt.savefig(os.path.join(plot_save_path, "wer_local_2_plot.png"))
plt.close()
